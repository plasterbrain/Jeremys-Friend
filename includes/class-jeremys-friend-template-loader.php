...
if ( ! class_exists( 'Jeremys_Friend_Template_Loader' ) ) {
	 ...
	 * @author  Gary Jones
	 */
	class Jeremys_Friend_Template_Loader {
		/**
		 ...
		 * @var string
		 */
		protected $filter_prefix = 'jeremys_friend';
		 ...
		 * @var string
		 */
		protected $theme_template_directory = 'partials';

		/**
		 * Reference to the root directory path of this plugin.
		 *
		 * Can either be a defined constant, or a relative reference from where the
		 * subclass lives.
		 *
		 ...
		 *
		 * Means unset_template_data() can remove all custom references from
		 * $wp_query.
		 *
		 ...
		 *
		 * Data is available to the template as properties under the `$data`
		 * variable. i.e. A value provided here under `$data['foo']` is available
		 * as `$data->foo`.
		 *
		 * When an input key has a hyphen, you can use `$data->{foo-bar}` in the
		 * template.
		 *
		 * @since 1.2.0
		 *
		 * @param mixed  $data     Custom data for the template.
		 * @param string $var_name Optional. Variable under which the custom data
		 *                                   is available in the template. Default
		 *                                   is 'data'.
		...
		protected function get_template_file_names( $slug, $name ) {
			 ...
			 * least specific last. e.g. 0 => recipe-instructions.php, 1 => recipe.php
			 *
			 * @since 1.0.0
			 *
			 * @param array  $templates Names of template files that should be looked 
			 *                          for, for given slug and name.
			 * @param string $slug      Template slug.
			 * @param string $name      Template variation name.
			 */
		 ...
		 * is not found in either of those, it looks in the theme-compat folder.
		 *
		 * @since 1.0.0
		 *
		 * @param string|array $template_names Template file(s) to search for, in
		 *                                     order.
		 * @param bool         $load           If true the template file will be
		 *                                     loaded if it is found.
		 * @param bool         $require_once   Whether to require_once or require.
		 *                                     Default true. Has no effect if $load
		 *                                     is false.
		 * @return string                      The template filename if one is
		 *                                     located.
		 */
		 ...
		 *
		 * Default is to check in a child theme (if relevant) before a parent
		 * theme, so that themes which inherit from a parent theme can just
		 * overload one file. If the template is not found in either of those, it
		 * looks in the theme-compat folder last.
		 *
		 * @since 1.0.0
		 *
		 * @return mixed|void
		...
		protected function get_template_paths() {
			 ...
			 * @since 1.0.0
			 *
			 * @param array $var Default is directory in child theme at index 1,
